...
bool ComportamientoJugador::pathFinding(int level, const estado &origen, const list<estado> &destino, list<Action> &plan)
{
	...
	case 3:
		...
		if (positionToGo.first != -1 and positionToGo.second != -1)
		{
			...
			(mapaResultado[subObjetivos.front().fila][subObjetivos.front().columna] == 'B') or
			(mapaResultado[subObjetivos.front().fila][subObjetivos.front().columna] == 'M'))
			{
...
void ComportamientoJugador::busquedaPuntoLejano(estado current, int nivel)
{
	...
	case 7:
		...
		for (int i = current.fila; i >= 3 and encontrado == false; i--)
		{
		...
		if (encontrado == true)
		{
			...
			if(inTablero(aux))
				if(mapaResultado[aux.fila][aux.columna] != 'A' and mapaResultado[aux.fila][aux.columna] != 'B' and mapaResultado[aux.fila][aux.columna] != 'M')
...
bool ComportamientoJugador::pathFinding_A(const estado &origen, const estado &destino, list<Action> &plan)
{
	...
	frontier = priority_queue<nodo, vector<nodo>, ComparaFuncionCoste>();
